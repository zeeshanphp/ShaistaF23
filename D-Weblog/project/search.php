...
$message = "";
$error = "";
$result = mysqli_query($conn, "SELECT * FROM blogs b JOIN users u ON b.writer=u.user_id");
if (isset($_POST['like'])) {
    $blog = $_POST['blog'];
...
<div class="row">
    <div class="col-md-12 bg-info py-3">
        <center>
            <h4 class="text-white">SEARCH BLOG</h4>
        </center>
    </div>
</div>
<div class="row">
    ...
    <div class="col-md-8">
        ...
        <form method="post">
            <table class="table">
                <tr>
                    ...
                    <td><input type="submit" name="nsearch" value="SEARCH BLOGS" class="btn btn-info w-100"></td>
                </tr>
            </table>
        </form>
        <?php
        if (isset($_POST['nsearch'])) {
            $find = $_POST['nfind'];
            $query = "select * from blogs b JOIN users u ON b.writer=u.user_id where b.bcat LIKE '$find%'";
        ...
        ?>
                ...
                <div class="alert-danger alert">No Record Found</div>
        <?php    }
        } ?>
    </div>
</div>
